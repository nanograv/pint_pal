...
# marker dictionary as well, may not need/want this...
markers = {"327_ASP":        ".",
          "327_PUPPI":       "s",
          "430_ASP":         ".",
          "430_PUPPI":       "s",
...
def plot_residuals_time(toas, res, figsize=(10,4), plotsig = False, fromPINT = True, NB = False, errs = None,\
                        mjds = None, rcvr_bcknds = None, avg = False, whitened = False, save = False, \
    ...
    if save:
        ...
        if NB:
            ...
            ext += "_WB"
        plt.savefig("resid_v_mjd%s.png" % (ext))
    return
...
def plot_residuals_orb(toas, res, model, figsize=(10,4), plotsig = False, fromPINT = True, errs = None,\
                        mjds = None, rcvr_bcknds = None, avg = False, whitened = False, save = False, \
                       legend = True, axs = None):
    """
    ...
    avg: boolean, if True will change y-axis to "Average Residual"
    whitened: boolean, if True will change y-axis to "Residual (Whitened)"
    save: boolean, if True will save plot with the name "resid_v_orbphase.png" Will add averaged/whitened
         as necessary
    ...
    binary_model_name = 'Binary'+model.binary_model_name
    # Now get the phases in units of orbits
    delay = model.delay(toas)
    orbit_phase = model.components[binary_model_name].binary_instance.orbits()
    ...
    phases = np.abs(np.modf(orbit_phase)[0])

    """Method to get phases for averaged or DMX residuals. This is not exactly the right way to do this,
    and will need to fixed once a better method has been written in PINT.

    NOTE - Still need to test this..."""
    if avg:
        """
        # Start by epoch averaging the residuals to get the right TOA indices
        avg = fitter.resids.ecorr_average(use_noise_model=True)
        # Now get indices of orbital phases and backends
        avg_phases = []
        avg_rcvr_bcknds = []
        print(len(avg['indices']))
        for iis in avg['indices']:
            avg_phases.append(np.mean(phases[iis]))
            avg_rcvr_bcknds.append(rcvr_bcknds[iis[0]])
        phases = np.array(avg_phases)
        rcvr_bcknds = np.array(avg_rcvr_bcknds)
        print(len(phases), len(rcvr_bcknds))
        """
        log.warn("Currently cannot produce time averaged residuals v. orbital phase...")
    ...
    for i, r_b in enumerate(RCVR_BCKNDS):
        ...
        if plotsig:
            sig = res[inds]/errs[inds]
            ax1.errorbar(phases[inds], sig, yerr=len(errs[inds])*[1], fmt=markers[r_b_label], \
                     color=colorscheme[r_b_label], label=r_b_label)
        else:
            ax1.errorbar(phases[inds], res[inds], yerr = errs[inds], fmt=markers[r_b_label], \
                     color=colorscheme[r_b_label], label=r_b_label)
    # Set second axis
    ...
    if save:
        ...
        if avg:
            ext += "_averaged"
        plt.savefig("resid_v_orbphase%s.png" % (ext))
    return
